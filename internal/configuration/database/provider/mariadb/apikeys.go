...
func (p DatabaseProvider) GetAllApiKeys() map[string]models.ApiKey {
	...
	rows, err := p.sqlDb.Query("SELECT * FROM ApiKeys WHERE ApiKeys.Expiry = 0 OR ApiKeys.Expiry > ?", currentTime().Unix())
	helper.Check(err)
	defer rows.Close()
	for rows.Next() {
...
func (p DatabaseProvider) GetApiKey(id string) (models.ApiKey, bool) {
	...
	row := p.sqlDb.QueryRow("SELECT * FROM ApiKeys WHERE Id = ?", id)
...
func (p DatabaseProvider) GetApiKeyByPublicKey(publicKey string) (string, bool) {
	var rowResult schemaApiKeys
	row := p.sqlDb.QueryRow("SELECT Id FROM ApiKeys WHERE PublicId = ? LIMIT 1", publicKey)
...
func (p DatabaseProvider) SaveApiKey(apikey models.ApiKey) {
	...
	_, err := p.sqlDb.Exec("REPLACE INTO ApiKeys (Id, FriendlyName, LastUsed, Permissions, Expiry, IsSystemKey, UserId, PublicId, UploadRequestId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
func (p DatabaseProvider) UpdateTimeApiKey(apikey models.ApiKey) {
	_, err := p.sqlDb.Exec("UPDATE ApiKeys SET LastUsed = ? WHERE Id = ?",
...
func (p DatabaseProvider) DeleteApiKey(id string) {
	_, err := p.sqlDb.Exec("DELETE FROM ApiKeys WHERE Id = ?", id)
	helper.Check(err)
}

func (p DatabaseProvider) cleanApiKeys() {
	_, err := p.sqlDb.Exec("DELETE FROM ApiKeys WHERE ApiKeys.Expiry > 0 AND ApiKeys.Expiry < ?", currentTime().Unix())
	helper.Check(err)
}
