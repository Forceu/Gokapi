...
func (p DatabaseProvider) GetMetaDataById(id string) (models.File, bool) {
	...
	row := p.sqlDb.QueryRow("SELECT * FROM FileMetaData WHERE Id = ?", id)
...
func (p DatabaseProvider) SaveMetaData(file models.File) {
	...
	_, err = p.sqlDb.Exec(`REPLACE INTO FileMetaData (Id, Name, Size, SHA1, ExpireAt, SizeBytes,
...
func (p DatabaseProvider) IncreaseDownloadCount(id string, decreaseRemainingDownloads bool) {
	if decreaseRemainingDownloads {
		...
		_, err := p.sqlDb.Exec(`UPDATE FileMetaData SET DownloadCount = DownloadCount + 1 WHERE Id = ?`, id)
		helper.Check(err)
	}
}
...
func (p DatabaseProvider) GetDownloadsRemaining(id string) int {
	...
	row := p.sqlDb.QueryRow("SELECT DownloadsRemaining FROM FileMetaData WHERE Id = ?", id)
...
func (p DatabaseProvider) DeleteMetaData(id string) {
	_, err := p.sqlDb.Exec("DELETE FROM FileMetaData WHERE Id = ?", id)
	helper.Check(err)
}
