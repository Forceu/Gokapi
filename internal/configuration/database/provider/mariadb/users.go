...
func (p DatabaseProvider) SaveUser(user models.User, isNewUser bool) {
	...
	if isNewUser {
		_, err := p.sqlDb.Exec("INSERT INTO Users (Name, Password, Permissions, Userlevel, LastOnline, ResetPassword) VALUES (?, ?, ?, ?, ?, ?)",
			user.Name, user.Password, user.Permissions, user.UserLevel, user.LastOnline, resetpw)
		helper.Check(err)
	} else {
		_, err := p.sqlDb.Exec("REPLACE INTO Users (Id, Name, Password, Permissions, Userlevel, LastOnline, ResetPassword) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
func (p DatabaseProvider) UpdateUserLastOnline(id int) {
	...
	_, err := p.sqlDb.Exec("UPDATE Users SET LastOnline = ? WHERE Id = ?", timeNow, id)
	helper.Check(err)
}

// DeleteUser deletes a user with the given ID
func (p DatabaseProvider) DeleteUser(id int) {
	_, err := p.sqlDb.Exec("DELETE FROM Users WHERE Id = ?", id)
	helper.Check(err)
}
