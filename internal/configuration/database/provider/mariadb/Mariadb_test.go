//go:build test && mariadbtest

// Package mariadb tests require a real MariaDB/MySQL server. They are gated behind the
// "mariadbtest" build tag (not part of the default "test" tag set) and configured via
// environment variables, since there is no pure-Go in-process mock for MariaDB/MySQL
// (unlike SQLite, which is just a file, or Redis, which uses miniredis):
//
//	GOKAPI_MARIADB_HOST=127.0.0.1:3306
//	GOKAPI_MARIADB_DBNAME=gokapi_test
//	GOKAPI_MARIADB_USER=gokapi
//	GOKAPI_MARIADB_PASSWORD=secret
//
// Run with: go test ./internal/configuration/database/provider/mariadb/... --tags=test,mariadbtest
//
...
var config = models.DbConnection{
	HostUrl:      envOrDefault("GOKAPI_MARIADB_HOST", "127.0.0.1:3306"),
	DatabaseName: envOrDefault("GOKAPI_MARIADB_DBNAME", "gokapi_test"),
	Username:     envOrDefault("GOKAPI_MARIADB_USER", "gokapi"),
	Password:     envOrDefault("GOKAPI_MARIADB_PASSWORD", "secret"),
...
func TestMain(m *testing.M) {
	err := dropAllTables()
	if err != nil {
		fmt.Println("Could not reset MariaDB test database:", err)
		os.Exit(1)
	}
	exitVal := m.Run()
	os.Exit(exitVal)
...
func TestInit(t *testing.T) {
	...
	_, err = New(models.DbConnection{HostUrl: "", DatabaseName: "gokapi_test", Type: 2})
	test.IsNotNil(t, err)
	_, err = New(models.DbConnection{HostUrl: config.HostUrl, DatabaseName: "", Type: 2})
	test.IsNotNil(t, err)
}
