...
func (p DatabaseProvider) GetSession(id string) (models.Session, bool) {
	...
	row := p.sqlDb.QueryRow("SELECT * FROM Sessions WHERE Id = ?", id)
...
func (p DatabaseProvider) SaveSession(id string, session models.Session) {
	...
	_, err := p.sqlDb.Exec("REPLACE INTO Sessions (Id, RenewAt, ValidUntil, UserId) VALUES (?, ?, ?, ?)",
...
func (p DatabaseProvider) DeleteSession(id string) {
	_, err := p.sqlDb.Exec("DELETE FROM Sessions WHERE Id = ?", id)
	helper.Check(err)
}
...
func (p DatabaseProvider) DeleteAllSessionsByUser(userId int) {
	_, err := p.sqlDb.Exec("DELETE FROM Sessions WHERE UserId = ?", userId)
	helper.Check(err)
}

func (p DatabaseProvider) cleanExpiredSessions() {
	_, err := p.sqlDb.Exec("DELETE FROM Sessions WHERE Sessions.ValidUntil < ?", time.Now().Unix())
	helper.Check(err)
}
