...
func (p DatabaseProvider) GetHotlink(id string) (string, bool) {
	...
	row := p.sqlDb.QueryRow("SELECT FileId FROM Hotlinks WHERE Id = ?", id)
...
func (p DatabaseProvider) SaveHotlink(file models.File) {
	...
	_, err := p.sqlDb.Exec("REPLACE INTO Hotlinks (Id, FileId) VALUES (?, ?)",
...
func (p DatabaseProvider) DeleteHotlink(id string) {
	...
	_, err := p.sqlDb.Exec("DELETE FROM Hotlinks WHERE Id = ?", id)
	helper.Check(err)
}
