...
func (p DatabaseProvider) GetAllUsers() []models.User {
	...
	rows, err := p.sqlDb.Query("SELECT * FROM users ORDER BY userlevel, lastonline DESC, name")
	helper.Check(err)
	defer rows.Close()
	for rows.Next() {
		row := schemaUser{}
...
func (p DatabaseProvider) getUserWithConstraint(isName bool, searchValue any) (models.User, bool) {
	...
	row := p.sqlDb.QueryRow(query, searchValue)
...
func (p DatabaseProvider) SaveUser(user models.User, isNewUser bool) {
	...
	if isNewUser {
		_, err := p.sqlDb.Exec("INSERT INTO users (name, password, permissions, userlevel, lastonline, resetpassword) VALUES ($1, $2, $3, $4, $5, $6)",
			user.Name, user.Password, user.Permissions, user.UserLevel, user.LastOnline, resetpw)
		helper.Check(err)
	} else {
		_, err := p.sqlDb.Exec(`INSERT INTO users (id, name, password, permissions, userlevel, lastonline, resetpassword)
			VALUES ($1, $2, $3, $4, $5, $6, $7)
			ON CONFLICT (id) DO UPDATE SET name = $2, password = $3, permissions = $4, userlevel = $5, lastonline = $6, resetpassword = $7`,
...
func (p DatabaseProvider) UpdateUserLastOnline(id int) {
	...
	_, err := p.sqlDb.Exec("UPDATE users SET lastonline = $1 WHERE id = $2", timeNow, id)
	helper.Check(err)
}

// DeleteUser deletes a user with the given ID
func (p DatabaseProvider) DeleteUser(id int) {
	_, err := p.sqlDb.Exec("DELETE FROM users WHERE id = $1", id)
	helper.Check(err)
}
