//go:build test && postgrestest

// Package postgres tests require a real PostgreSQL server. They are gated behind the
// "postgrestest" build tag (not part of the default "test" tag set) and configured via
// environment variables, since there is no pure-Go in-process mock for PostgreSQL
// (unlike SQLite, which is just a file, or Redis, which uses miniredis):
//
//	GOKAPI_POSTGRES_HOST=127.0.0.1:5432
//	GOKAPI_POSTGRES_DBNAME=gokapi_test
//	GOKAPI_POSTGRES_USER=gokapi
//	GOKAPI_POSTGRES_PASSWORD=secret
//
// Run with: go test ./internal/configuration/database/provider/postgres/... --tags=test,postgrestest
//
// The target database must already exist; the tables in it are dropped and recreated on
// every run, so use a dedicated, disposable database - never point this at production data.
package postgres
...
var config = models.DbConnection{
	HostUrl:      envOrDefault("GOKAPI_POSTGRES_HOST", "127.0.0.1:5432"),
	DatabaseName: envOrDefault("GOKAPI_POSTGRES_DBNAME", "gokapi_test"),
	Username:     envOrDefault("GOKAPI_POSTGRES_USER", "gokapi"),
	Password:     envOrDefault("GOKAPI_POSTGRES_PASSWORD", "secret"),
...
func dropAllTables() error {
	...
	tables := []string{"apikeys", "e2econfig", "filemetadata", "hotlinks", "sessions", "users", "uploadrequests", "statistics", "schemaversion"}
	for _, table := range tables {
		_, err = instance.sqlDb.Exec("DROP TABLE IF EXISTS " + table)
		if err != nil {
			return err
		}
	}
...
func TestMain(m *testing.M) {
	err := dropAllTables()
	if err != nil {
		fmt.Println("Could not reset PostgreSQL test database:", err)
		os.Exit(1)
	}
	exitVal := m.Run()
	os.Exit(exitVal)
...
func TestInit(t *testing.T) {
	...
	_, err = New(models.DbConnection{HostUrl: "", DatabaseName: "gokapi_test", Type: 3})
	test.IsNotNil(t, err)
	_, err = New(models.DbConnection{HostUrl: config.HostUrl, DatabaseName: "", Type: 3})
	test.IsNotNil(t, err)
}
...
func TestDatabaseProvider_GetType(t *testing.T) {
	test.IsEqualInt(t, dbInstance.GetType(), 3)
}
...
func TestMetaData(t *testing.T) {
	...
	dbInstance.DeleteMetaData("test2")
	dbInstance.DeleteMetaData("test3")
}
...
func TestHotlink(t *testing.T) {
	...
	dbInstance.DeleteHotlink("testlink")
	dbInstance.DeleteHotlink("link2")
	dbInstance.DeleteHotlink("link3")
}
...
func TestApiKey(t *testing.T) {
	...
	dbInstance.DeleteApiKey("expiredKey")
}
...
func TestFileRequest(t *testing.T) {
	...
	ids := []string{requests[0].Id, requests[1].Id, requests[2].Id}
	...
	dbInstance.DeleteFileRequest(req1)
	dbInstance.DeleteFileRequest(req2)
	dbInstance.DeleteFileRequest(req3)
}
...
func TestUpdateTimeApiKey(t *testing.T) {
	...
	dbInstance.DeleteApiKey("key1")
	dbInstance.DeleteApiKey("key2")
	dbInstance.DeleteApiKey("publicTest")
}
...
func TestParallelConnectionsReading(t *testing.T) {
	...
	wg.Wait()
	dbInstance.DeleteApiKey("readtest")
}

func TestStatistics(t *testing.T) {
	test.IsEqualInt64(t, int64(dbInstance.GetStatTraffic()), 0)
	dbInstance.SaveStatTraffic(1024)
	test.IsEqualInt64(t, int64(dbInstance.GetStatTraffic()), 1024)
	dbInstance.SaveStatTraffic(2048)
	test.IsEqualInt64(t, int64(dbInstance.GetStatTraffic()), 2048)

	_, ok := dbInstance.GetTrafficSince()
	test.IsEqualBool(t, ok, false)
	dbInstance.SaveTrafficSince(12345)
	since, ok := dbInstance.GetTrafficSince()
	test.IsEqualBool(t, ok, true)
	test.IsEqualInt64(t, since, 12345)
	dbInstance.SaveTrafficSince(54321)
	since, ok = dbInstance.GetTrafficSince()
	test.IsEqualBool(t, ok, true)
	test.IsEqualInt64(t, since, 54321)
}
