...
func (p DatabaseProvider) GetAllMetadata() map[string]models.File {
	...
	rows, err := p.sqlDb.Query("SELECT * FROM filemetadata")
	helper.Check(err)
	defer rows.Close()
	for rows.Next() {
...
func (p DatabaseProvider) GetMetaDataById(id string) (models.File, bool) {
	...
	row := p.sqlDb.QueryRow("SELECT * FROM filemetadata WHERE id = $1", id)
...
func (p DatabaseProvider) SaveMetaData(file models.File) {
	...
	_, err = p.sqlDb.Exec(`INSERT INTO filemetadata (id, name, size, sha1, expireat, sizebytes,
          ...
          ON CONFLICT (id) DO UPDATE SET name = $2, size = $3, sha1 = $4, expireat = $5, sizebytes = $6,
                downloadsremaining = $7, downloadcount = $8, passwordhash = $9, hotlinkid = $10, contenttype = $11,
                awsbucket = $12, encryption = $13, unlimiteddownloads = $14, unlimitedtime = $15, userid = $16,
                uploaddate = $17, pendingdeletion = $18, uploadrequestid = $19`,
...
func (p DatabaseProvider) IncreaseDownloadCount(id string, decreaseRemainingDownloads bool) {
	if decreaseRemainingDownloads {
		_, err := p.sqlDb.Exec(`UPDATE filemetadata SET downloadcount = downloadcount + 1,
                        downloadsremaining = downloadsremaining - 1 WHERE id = $1`, id)
		helper.Check(err)
	} else {
		_, err := p.sqlDb.Exec(`UPDATE filemetadata SET downloadcount = downloadcount + 1 WHERE id = $1`, id)
		helper.Check(err)
	}
}
...
func (p DatabaseProvider) GetDownloadsRemaining(id string) int {
	...
	row := p.sqlDb.QueryRow("SELECT downloadsremaining FROM filemetadata WHERE id = $1", id)
...
func (p DatabaseProvider) DeleteMetaData(id string) {
	_, err := p.sqlDb.Exec("DELETE FROM filemetadata WHERE id = $1", id)
	helper.Check(err)
}
